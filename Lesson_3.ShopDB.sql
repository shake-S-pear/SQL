...
VALUES
(1,'Иван', 'Иванович', 'Белецкий', 2000, 0, '2009-11-20', NULL, 1),
(2,'Петр', 'Григорьевич', 'Дяченко', 1000, 0, '2009-11-20', NULL, 2),
(3,'Светлана', 'Олеговна', 'Лялечкина', 800, 0, '2009-11-20', NULL, 2);
...
select e.FName, e.MName, e.LName, sum(TotalPrice) as TotalSold, 'employee' as job from Employees e
join Orders o on e.EmployeeID = o.EmployeeID
join OrderDetails od on o.OrderID = od.OrderID
group by e.FName, e.MName, e.LName
having sum(TotalPrice) > 1000
union
select c.FName, c.MName, c.LName, sum(TotalPrice) as TotalSold, 'customer' as job  from Customers c
join Orders o on c.CustomerNo = o.CustomerNo
join OrderDetails od on o.OrderID = od.OrderID
group by c.FName, c.MName, c.LName
having sum(TotalPrice) > 1000;

-- Используя JOIN’s и ShopDB получить имена покупателей и имена сотрудников у которых TotalPrice товара больше 500
